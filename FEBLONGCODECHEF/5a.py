...
def find_no(list1,list2):
   xxval=len(set(list1+list2))
   return xxval;
...
for qq in range(int(input())):
   ...
   for index in label:
      x_val=index[1:]
      if x_val in blabel:
          blabel[x_val].append(index[0])
      else:
          blabel[x_val]=[index[0]]
   blabel1=list(blabel.keys())
   cvalue=0
   for i in range(len(blabel)):
       for j in range(i+1,len(blabel)):
           yy=find_no(blabel[blabel1[i]],blabel[blabel1[j]])
           
           cvalue=cvalue+(yy-len(blabel[blabel1[i]]))*(yy-len(blabel[blabel1[j]]))
